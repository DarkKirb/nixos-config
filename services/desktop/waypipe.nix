  ...
  systemd.user.services = {
    waypipe-client = {
      ...
      Service = {
        ...
        ExecStart = "${lib.getExe pkgs.waypipe} --video=bpf=8000000 --socket %h/.local/state/waypipe/client.sock client";
    ...
    waypipe-server = {
      ...
      Service = {
        Type = "simple";
        ExecStartPre = "${lib.getExe' pkgs.coreutils "mkdir"} %h/.local/state/waypipe -p";
        ExecStart = "${lib.getExe pkgs.waypipe} --video=bpf=8000000 --socket %h/.local/state/waypipe/server.sock --title-prefix '[%H] ' --login-shell --display wayland-waypipe server -- ${lib.getExe' pkgs.coreutils "sleep"} infinity";
